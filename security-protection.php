...
if ( ! function_exists( 'secprot_form_inputs' ) ) :
	...
	function secprot_form_inputs() {
		...
		echo '<input type="text" name="secprot-code" class="input" value="" />';
		echo '</label></p>'."\n";
		...
		if (!empty($_POST)) {
			...
			if (!is_wp_error($user)) { // user gave us valid username and password
				if ($secprot_arr['send_successful_login_log_to_admin']) {
					secprot_log('Login successful: passed Security-protection check; correct username and password;');
				}
	...
	function secprot_log( $error_message = '' ) {
		global $secprot_arr;
		if (!empty($_POST)) {
			...
			//$secprot_message_brute_force_info .= 'SERVER_PROTOCOL : ' . $_SERVER['SERVER_PROTOCOL'] . "\r\n";
			//$secprot_message_brute_force_info .= 'REDIRECT_STATUS : ' . $_SERVER['REDIRECT_STATUS'] . "\r\n\r\n";
			...
			/*$secprot_message_brute_force_info .= 'SERVER vars:'."\r\n"; // lets see what SERVER vars brute-forcers try to submit
			foreach ( $_SERVER as $key => $value ) {
				$secprot_message_brute_force_info .= '$_SERVER['.$key. '] = '.$value."\r\n"; // .chr(13).chr(10)
			}
			$secprot_message_brute_force_info .= "\r\n\r\n";*/

			/*$secprot_message_brute_force_info .= 'ENV vars:'."\r\n"; // lets see what ENV vars brute-forcers try to submit
			foreach ( $_ENV as $key => $value ) {
				$secprot_message_brute_force_info .= '$_ENV['.$key. '] = '.$value."\r\n"; // .chr(13).chr(10)
			}
			$secprot_message_brute_force_info .= "\r\n\r\n";*/
...
if ( ! function_exists( 'secprot_plugin_meta' ) ) :
	function secprot_plugin_meta( $links, $file ) { // add 'Plugin page' and 'Donate' links to plugin meta row
